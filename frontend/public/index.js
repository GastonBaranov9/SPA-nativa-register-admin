  ...
  lista.innerHTML = usuarios
    .map(
      ...
      <li>
        ...
        <button class="editarUsuario" data-id="${u.id_usuario}" data-nombre="${u.nombre}" data-apellido="${u.apellido}" data-edad="${u.edad}" data-password="${u.password}">
          Modificar usuario
        </button>
      </li>`
    )
...
async function guardarUsuario(event) {
  ...
  const password = crear.elements["password"].value;
  const isAdmin = !!crear.elements["isAdmin"].checked;
  crear.reset();
  await create({ nombre, apellido, edad, password, isAdmin });
  await listarUsuario();
}
...
function editarUsuario(idUsuario, nombre, apellido, edad, password, isAdmin) {
  ...
  document.getElementById("nuevaPass-modificar").value = password || "";
  document.getElementById("nuevoRol-modificar").checked =
    String(isAdmin) === "true";
}
...
async function guardarEdicionUsuario(event) {
  ...
  const nuevonombre = document
    .getElementById("nuevoNombre-modificar")
    .value.trim();
  const nuevoapellido = document
    .getElementById("nuevoApellido-modificar")
    .value.trim();
  const nuevaedad =
    Number(document.getElementById("nuevaEdad-modificar").value) || null;
  const nuevaPass = document.getElementById("nuevaPass-modificar").value.trim();
  const isAdmin = document.getElementById("nuevoRol-modificar").checked;
...
lista.addEventListener("click", async (e) => {
  ...
  if (editarBtn) {
    const { id, nombre, apellido, edad, password, isAdmin } = editarBtn.dataset;
    return editarUsuario(id, nombre, apellido, edad, password, isAdmin);
  }
...
async function registrarUsuario(event) {
  ...
  const isAdmin = !!registerForm.elements["isAdmin"].checked;

  try {
    const res = await register({ nombre, apellido, edad, password, isAdmin });
    registerForm.reset();
    await listarUsuario();
  } catch (err) {
    console.error(err);
...
botonRegister.addEventListener("click", () => {
  registerDiv.style.display =
    registerDiv.style.display === "none" ? "block" : "none";
});
